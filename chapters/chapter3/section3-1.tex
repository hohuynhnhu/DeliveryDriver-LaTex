...
\subsection{Phân hệ Quản lý RFM}

\begin{figure}[H]
    \centering
    \includegraphics[height=0.3\textheight]{frontmatter/image/rfm-management.jpg}
    \caption{Sơ đồ phân rã chức năng Quản lý RFM}
    \label{fig:rfm-management}
\end{figure}

Dựa trên Hình~\ref{fig:rfm-management}, phân hệ \textbf{Quản lý RFM} chịu trách nhiệm phân tích và đánh giá hành vi khách hàng dựa trên mô hình RFM (Recency, Frequency, Monetary). 
Phân hệ này hỗ trợ nhà quản lý theo dõi mức độ tương tác, tần suất sử dụng dịch vụ và giá trị giao dịch của khách hàng, từ đó phục vụ cho việc phân nhóm khách hàng và ra quyết định kinh doanh.

\subsubsection{Xem RFM}

Chức năng này cho phép người quản trị theo dõi thông tin RFM của khách hàng:
\begin{itemize}
    \item \textbf{Xem danh sách khách hàng}: Hiển thị danh sách khách hàng kèm theo các chỉ số RFM.
    \item \textbf{Xem chi tiết RFM}: Cung cấp thông tin chi tiết về Recency, Frequency và Monetary của từng khách hàng.
\end{itemize}

\subsubsection{Phân tích RFM}

Chức năng này hỗ trợ phân tích dữ liệu RFM nhằm đánh giá hành vi khách hàng:
\begin{itemize}
    \item \textbf{Phân nhóm khách hàng}: Phân loại khách hàng theo mức độ gắn bó và giá trị.
    \item \textbf{Đánh giá hành vi}: Phân tích xu hướng sử dụng dịch vụ của khách hàng theo thời gian.
\end{itemize}

\subsubsection{Tính toán RFM}

Chức năng này thực hiện tính toán các chỉ số RFM dựa trên dữ liệu giao dịch:
\begin{itemize}
    \item \textbf{Tính Recency}: Xác định khoảng thời gian kể từ lần giao dịch gần nhất.
    \item \textbf{Tính Frequency}: Xác định tần suất thực hiện giao dịch.
    \item \textbf{Tính Monetary}: Tính tổng giá trị giao dịch của khách hàng.
\end{itemize}

\subsubsection{Cập nhật RFM}

Chức năng này cho phép cập nhật lại các chỉ số RFM khi dữ liệu thay đổi:
\begin{itemize}
    \item \textbf{Cập nhật dữ liệu}: Làm mới chỉ số RFM dựa trên giao dịch mới.
    \item \textbf{Đồng bộ thông tin}: Đảm bảo dữ liệu phân tích luôn chính xác và nhất quán.
\end{itemize}



>>>>>>> 610c63ea0f72362b1da851204eb3996f084c21bd
