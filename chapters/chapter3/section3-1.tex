...
\begin{tikzpicture}[
    ...
    every node/.style={draw, rectangle, rounded corners, align=center, minimum width=3.4cm, minimum height=0.9cm},
...
% ===== API Gateway =====
\node (gateway) [right=1.2cm of driver] {API Gateway};

% ===== Backend Services =====
\node (auth) [right=1.2cm of gateway, yshift=2.3cm] {Authentication\\Service};
...
% ===== Database =====
\node (db) [right=1.5cm of schedule] {Supabase\\PostgreSQL + Auth};
