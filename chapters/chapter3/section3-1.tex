...
% --- UC-22: PHÂN TÍCH CẶP ĐIỂM ---
\begin{table}[H]
\centering
\caption{UC-22: Phân tích cặp điểm giao nhận }
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|p{3.5cm}|p{11cm}|}
\hline
\textbf{Tên Use Case} & Phân tích cặp điểm giao nhận \\ \hline
\textbf{Actor} & Admin  \\ \hline
\textbf{Mô tả} & Hệ thống sử dụng kỹ thuật để khai phá dữ liệu, tìm ra mối tương quan và tần suất xuất hiện cùng nhau giữa điểm lấy hàng (Pickup) và điểm giao hàng (Drop), từ đó nhận diện các "tuyến đường nóng". \\ \hline
\textbf{Tiền điều kiện} & Dữ liệu lịch sử đơn hàng (gồm tọa độ nhận/giao) đủ lớn và đã được làm sạch. \\ \hline
\textbf{Luồng chính} &
\begin{enumerate}
    \item Actor chọn module phân tích vận hành.
    \item Actor chọn chức năng phân tích luồng tuyến.
    \item Actor thiết lập tham số đầu vào: Khoảng thời gian, Loại xe, và Ngưỡng hỗ trợ tối thiểu .
    \item Hệ thống chạy thuật toán Market Basket Analysis trên tập dữ liệu đã lọc.
    \item Hệ thống hiển thị kết quả
\end{enumerate} \\ \hline
\textbf{Luồng mở rộng} &
\textbf{Tại bước 5 :} Nếu một cặp điểm có chỉ số Lift cao vượt ngưỡng nhưng chưa có bưu cục trung gian:
\begin{enumerate}
    \item[5a.] Hệ thống thực hiện Use Case mở rộng: "Gợi ý mở bưu cục mới".
    \item[5b.] Hệ thống đánh dấu vị trí tiềm năng trên bản đồ và hiển thị dự báo hiệu quả nếu mở bưu cục tại đó.
\end{enumerate} \\ \hline
\textbf{Hậu điều kiện} & Admin nắm được xu hướng di chuyển của đơn hàng và nhận được các đề xuất mở rộng mạng lưới (nếu có). \\ \hline
\end{tabular}
\end{table}
