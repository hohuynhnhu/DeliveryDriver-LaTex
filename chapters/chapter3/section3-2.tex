...
\subsection{Activity Diagram phân công đơn hàng}

\begin{figure}[H]
    \centering
    \includegraphics[height=0.4\textheight]{frontmatter/image/Phancongdonhangactivity.jpg}
    \caption{Activity Diagram phân công đơn hàng}
    \label{fig:phancongdonhang}
\end{figure}

\textbf{Mục đích}

Activity Diagram ``Phân công đơn hàng'' được xây dựng nhằm mô tả quy trình nghiệp vụ khi Quản trị viên (Admin) thực hiện điều phối đơn hàng cho tài xế.
Sơ đồ làm rõ logic hệ thống trong việc tìm kiếm tài xế phù hợp, cơ chế kiểm tra tải trọng công việc (workload) để tránh quá tải, và các quy tắc xử lý ngoại lệ như gán đè (Force Assign) hoặc xử lý khi không tìm thấy tài xế.
Qua đó, sơ đồ giúp đảm bảo quy trình giao việc diễn ra minh bạch và tối ưu hóa hiệu suất vận hành.

\noindent\textbf{Luồng hoạt động chính}

Luồng hoạt động chính của chức năng phân công đơn hàng được mô tả như sau:

\begin{enumerate}
    \item Admin truy cập danh sách các đơn hàng chưa được phân công.
    \item Admin chọn một hoặc nhiều đơn hàng (thường trong cùng một khu vực) để tiến hành gán.
    \item Hệ thống tự động tìm kiếm và đề xuất danh sách tài xế phù hợp dựa trên vị trí và trạng thái.
    \item Hệ thống kiểm tra kết quả tìm kiếm:
    \begin{itemize}
        \item \textbf{Nếu không có tài xế phù hợp:} Hệ thống cảnh báo ``Tất cả tài xế đều bận hoặc ở quá xa''. Admin có thể quyết định ``Chờ đợi'' (kết thúc quy trình) hoặc ``Gán cho tài xế lân cận'' (tiếp tục quy trình).
        \item \textbf{Nếu có tài xế phù hợp:} Tiếp tục sang bước chọn tài xế.
    \end{itemize}
    \item Admin chọn một tài xế cụ thể từ danh sách để gán đơn.
    \item Hệ thống kiểm tra tải trọng công việc hiện tại của tài xế đó:
    \begin{itemize}
        \item \textbf{Nếu tài xế quá tải:} Hệ thống hiển thị cảnh báo ``Đã nhận quá số lượng đơn cho phép''. Admin cần xác nhận: chọn ``Gán đè'' (Force Assign) để tiếp tục hoặc ``Chọn người khác'' để quay lại bước chọn tài xế.
        \item \textbf{Nếu tải trọng bình thường:} Quy trình tiếp tục sang bước cập nhật.
    \end{itemize}
    \item Hệ thống cập nhật trạng thái đơn hàng thành ``Đã phân công''.
    ...
    \item Kết thúc quá trình phân công đơn hàng.
\end{enumerate}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.4\textheight]{frontmatter/image/Quanlyxulysucoactivity.jpg}
...
Activity Diagram ``Quản lý và xử lý sự cố đơn hàng'' được xây dựng nhằm mô tả quy trình can thiệp thủ công của Quản trị viên (Admin) khi xảy ra các tình huống ngoại lệ trong vận hành.
Sơ đồ chi tiết hóa các luồng xử lý phức tạp như: giải quyết đơn hàng giao thất bại, hỗ trợ khách hàng thay đổi địa chỉ giao hàng, hoặc cập nhật trạng thái đơn hàng thủ công.
Qua đó, sơ đồ giúp định nghĩa rõ ràng logic nghiệp vụ để đảm bảo mọi sự cố đều được giải quyết triệt để, đúng quy trình và đồng bộ dữ liệu chính xác trên hệ thống.

\noindent\textbf{Luồng hoạt động chính}

Luồng hoạt động chính của chức năng xử lý sự cố đơn hàng được mô tả như sau:
...
\begin{enumerate}
    ...
    \item Hệ thống thực hiện cập nhật trạng thái mới cho đơn hàng vào cơ sở dữ liệu.
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.4\textheight]{frontmatter/image/Thuchienlayhangactivity.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.4\textheight]{frontmatter/image/Thuchiengiaohangactivity.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.4\textheight]{frontmatter/image/Xemthongbaoactivity.jpg}
...
\subsection{Activity Diagram tối ưu hoá phân bổ lịch trình}

\begin{figure}[H]
    \centering
    \includegraphics[height=0.4\textheight]{frontmatter/image/ToiUuHoaPhanBoLichTrinh-AD.jpeg}
    \caption{Activity Diagram tối ưu hoá phân bổ lịch trình}
    \label{fig:toiuuhoaphanbolichtrinh}
\end{figure}